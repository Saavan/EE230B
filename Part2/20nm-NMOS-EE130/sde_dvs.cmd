...
(sdegeo:create-rectangle 
  ...
  (position    Xg Ygox 0.0 )  "HfO2" "R.Gateox" )

; Creating spacers regions
(sdegeo:create-rectangle 
  (position   (- Xg LOxsp)  (- 0.0 LOxsp) 0.0 ) 
  (position    Xsp  Ypol 0.0 )   "Si3N4" "R.Spacer" )

; Creating oxide spacers
(sdegeo:create-rectangle
  (position   Xg 0.0 0.0 )
  (position   (+ Xg LOxsp) Ypol 0.0)  "SiO2" "R.OxSpacer" )

(sdegeo:create-rectangle
  (position (+ Xg LOxsp) 0.0 0.0 )
  (position Xsp (- 0.0 LOxsp) 0.0) "SiO2" "R.OxSpacer" ) 

; Creating Metal gate
(sdegeo:create-rectangle 
  (position    0.0 Ygox 0.0 ) 
  (position    Xg  Ypol 0.0 )   "TiN" "R.MetalGate" )

; Create Silicide
(sdegeo:create-rectangle 
  (position    (+ Xsp  0.001) 0.0  0.0) 
  (position    (- Xmax 0.001) (* -1 XNiSi) 0.0) "NickelSilicide" "R.Silicide") 
;----------------------------------------------------------------------
; - rounding spacer
(sdegeo:fillet-2d 
...
(sdegeo:define-2d-contact 
 (find-edge-id (position (* (+ Xmax Xsp)  0.5)  (* -1 XNiSi) 0.0))  "drain")
...
(sde:addmaterial
  (find-body-id (position (* -0.5 (+ Xsp Xg)) (- LOxsp 0.01) 0.0))
   "SiO2" "R.OxSpacerLeft" )
...
(sde:addmaterial
  (find-body-id (position (* 0.5 (+ Xsp Xmax)) (* XNiSi -0.5) 0.0))
  "NickelSilicide" "R.SilicideRight") 

(sde:addmaterial
  (find-body-id (position (* -0.5 (+ Xsp Xmax)) (* XNiSi 0.5) 0.0))
  "NickelSilicide" "R.SilicideLeft")
;----------------------------------------------------------------------
; Saving BND file
...
(sdedr:define-gaussian-profile "Impl.Extprof"
 DopSD "PeakPos" 0  "PeakVal" 9e19 "ValueAtDepth" @ChanDoping@ "Depth" XjExt "Erf"  "Factor" 0.5 )
; -- implant placement
...
(sdedr:define-gaussian-profile "Impl.SDprof"
 ...
 "ValueAtDepth" @ChanDoping@ "Depth" XjSD "Gauss"  "Factor" 0.5 )
; -- implant placement
...
(sdedr:define-refinement-window "BaseLine.Halo" "Line"  
 (position (+ Xos (/ Lsp 2)) 0.0 0.0)  
 (position (* Xmax 2.0) 0.0 0.0) )
; -- implant definition
(sdedr:define-gaussian-profile "Impl.Haloprof"
 DopSub 
 "PeakPos" XjSD  "PeakVal" 1e19
 "ValueAtDepth" 1e17 "Depth" XjExt "Erf"  "Factor" @GaussianFactor@ )
; -- implant placement
